...
const API_FLOW: FlowConfig = {
  steps: [
    ...
    },
    {
      ...
      description: 'Choose which API endpoints to register as MCP tools. Publish them to a Gateway later from the Tool catalog.',
      action: 'save-tools',
      skippable: true,
...
const SERVER_FLOW: FlowConfig = {
  steps: [
    ...
    },
    {
      ...
      description: 'Review which tools from this MCP server to register. Publish them to a Gateway later from the Tool catalog.',
      action: 'save-tools',
      skippable: true,
      isDone: (ctx) => ctx.mcpMeta.exists || ctx.toolsCommitted,
      render: (ctx) => (
...
export const ONBOARD_FLOW: FlowConfig = {
  steps: [
    ...
    },
    {
      ...
      title: 'Select MCP Tools',
      description: 'Choose which tools exposed by this MCP server to register. Publish them to a Gateway later from the Tool catalog.',
      action: 'save-tools',
      skippable: true,
      isDone: (ctx) => ctx.mcpMeta.exists || ctx.toolsCommitted,
      render: (ctx) => (
