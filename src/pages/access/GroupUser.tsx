...
export default function GroupUser() {
  ...
  const taskContext = useMemo(
    ...
    [host, initialGroupId, initialUserId, searchContext],
  );
  ...
  const handleDelete = useCallback(async (row: MRT_Row<GroupUserType>) => {
    ...
    const cmd = {
      ...
      data: { hostId: row.original.hostId, groupId: row.original.groupId, userId: row.original.userId , aggregateVersion: row.original.aggregateVersion},
    };
  ...
  const handleUpdate = useCallback(async (row: MRT_Row<GroupUserType>) => {
    ...
    try {
      ...
      navigate(buildTaskAwareRoute('/app/form/updateGroupUser', searchParams, {
        ...taskContext,
        groupId,
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <Button
          ...
          onClick={() => navigate(
            buildTaskAwareRoute('/app/form/createGroupUser', searchParams, taskContext),
            { state: { data: { groupId: initialGroupId, userId: initialUserId } } },
          )}
        >
          Add User to Group
        </Button>
