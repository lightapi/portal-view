...
export default function PositionUser() {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<PositionUserType>) => {
    ...
    const cmd = {
      ...
      data: { hostId: row.original.hostId, positionId: row.original.positionId, userId: row.original.userId , aggregateVersion: row.original.aggregateVersion},
    };
  ...
  const table = useMaterialReactTable(usePortalActionTableOptions({
    ...
    renderRowActions: ({ row }) => <PortalActions row={row} actions={[
      {
        id: "update-position-user",
        label: "Update Position User",
        icon: (
          <SystemUpdateIcon />
        ),
      ...
      {
        id: "remove-user-from-position",
        label: "Remove User from Position",
