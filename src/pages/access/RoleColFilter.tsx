...
export default function RoleColFilter() {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<RoleColFilterType>) => {
    ...
    const cmd = {
      ...
      data: { hostId: row.original.hostId, roleId: row.original.roleId, endpointId: row.original.endpointId , aggregateVersion: row.original.aggregateVersion},
    };
  ...
  const handleUpdate = useCallback(async (row: MRT_Row<RoleColFilterType>) => {
    ...
    try {
      ...
      navigate(buildTaskAwareRoute('/app/form/updateRoleColFilter', searchParams, {
        ...taskContext,
        roleId,
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <Button
          ...
          onClick={() => navigate(
            buildTaskAwareRoute('/app/form/createRoleColFilter', searchParams, taskContext),
            {
              state: {
                data: {
                  roleId: initialRoleId,
