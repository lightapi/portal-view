...
export default function PositionColFilter() {
  ...
  const handleUpdate = useCallback(async (row: MRT_Row<PositionColFilterType>) => {
    ...
    try {
      ...
      navigate(buildTaskAwareRoute('/app/form/updatePositionColFilter', searchParams, {
        ...taskContext,
        positionId,
  ...
  const table = useMaterialReactTable(usePortalActionTableOptions({
    ...
    renderRowActions: ({ row }) => <PortalActions row={row} actions={[
      {
        id: "update-row-col-filter",
        label: "Update Row Col Filter",
        icon: (
          <SystemUpdateIcon />
        ),
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <Button
          ...
          onClick={() => navigate(
            buildTaskAwareRoute('/app/form/createPositionColFilter', searchParams, taskContext),
            {
              state: {
                data: {
                  positionId: initialPositionId,
