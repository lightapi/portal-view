...
export default function RuleAdmin() {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<RuleType>) => {
    ...
    const cmd = {
      ...
      data: { hostId: row.original.hostId, ruleId: row.original.ruleId , aggregateVersion: row.original.aggregateVersion},
    };
    ...
    renderRowActions: ({ row }) => <PortalActions row={row} actions={[
      {
        ...
        icon: <DetailsIcon />,
        onSelect: () => navigate(buildTaskAwareRoute('/app/ruleDetail', searchParams, contextForRow(row.original)), { state: { rule: row.original } })
      },
      {
        id: "update-rule",
        label: "Update Rule",
        ...
        loading: () => Boolean(isUpdateLoading === row.original.ruleId),
