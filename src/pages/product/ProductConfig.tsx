...
export default function ProductConfig() {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<ProductConfigType>) => {
    ...
    const cmd = {
      ...
      data: { hostId: row.original.hostId, productVersionId: row.original.productVersionId, configId: row.original.configId , aggregateVersion: row.original.aggregateVersion},
    };
  ...
  const table = useMaterialReactTable(usePortalActionTableOptions({
    ...
    renderRowActions: ({ row }) => <PortalActions row={row} actions={[
      {
        id: "delete-product-version-config",
        label: "Delete Product Version Config",
