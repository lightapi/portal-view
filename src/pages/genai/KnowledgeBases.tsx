...
import PortalEditIcon from '@mui/icons-material/Edit';
...
export default function KnowledgeBases() {
  ...
  return <PortalActionScope><Box sx={{ p: 2 }}>
    ...
    <Stack spacing={1}>{policies.map(policy => <Card variant="outlined" key={policy.ingestionPolicyId}><CardContent>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2}>
        ...
        {(policy.hostId || platformAdmin) && <PortalActions row={policy} actions={[
          { id: 'edit', label: policy.active ? 'Edit policy' : 'Review and reactivate policy', icon: <PortalEditIcon />, onSelect: () => openEditPolicy(policy) },
          { id: 'deactivate', label: 'Deactivate policy', icon: <PortalDeleteIcon />, destructive: true, hidden: () => !policy.active, onSelect: () => void deactivatePolicy(policy) },
        ]} />}
      </Stack>
    ...
    <Stack spacing={1}>{retrievalProfiles.map(profile => <Card variant="outlined" key={profile.profileId}><CardContent>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2}>
        ...
        {(profile.hostId || platformAdmin) && <PortalActions row={profile} actions={[
          { id: 'edit', label: profile.active ? 'Edit retrieval profile' : 'Review and reactivate retrieval profile', icon: <PortalEditIcon />, onSelect: () => openEditRetrieval(profile) },
          { id: 'deactivate', label: 'Deactivate retrieval profile', icon: <PortalDeleteIcon />, destructive: true, hidden: () => !profile.active, onSelect: () => void deactivateRetrieval(profile) },
        ]} />}
      </Stack>
    </CardContent></Card>)}</Stack>
    ...
    <Stack spacing={1}>{embeddingProfiles.map(profile => <Card variant="outlined" key={`${profile.profileId}:${profile.profileRevision}`}><CardContent>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2}>
        ...
        {platformAdmin && profile.active && <PortalActions row={profile} actions={[
          { id: 'deactivate', label: 'Deactivate embedding profile', icon: <PortalDeleteIcon />, destructive: true, onSelect: () => void deactivateEmbeddingProfile(profile) },
        ]} />}
      </Stack>
    </CardContent></Card>)}</Stack>
