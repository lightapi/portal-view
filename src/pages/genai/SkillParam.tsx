...
export default function SkillParam() {
    ...
    const handleUpdate = useCallback(async (row: MRT_Row<SkillParamType>) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(freshData.description || 'Failed to fetch latest parameter data.');
            }
    ...
    const columns = useMemo<MRT_ColumnDef<SkillParamType>[]>(
        () => [
            ...
            },
            {
                ...
                Cell: ({ row }) => (<Tooltip title="Delete Parameter"><IconButton color="error" onClick={() => handleDelete(row)}><DeleteForeverIcon /></IconButton></Tooltip>),
            },
        ],
