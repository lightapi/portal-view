...
describe('Pricing resource form navigation', () => {
  beforeEach(() => {
    ...
    mocks.listLlm.mockResolvedValue([{
      ...
      approvedBy:'finops@example.com',aggregateVersion:4,active:true,
      updateUser:'system',updateTs:'2026-08-01T12:00:00Z',
    }]);
  });

  it('opens create and update forms with only declared pricing fields', async () => {
    const pricing = llmAdminResources.find(resource => resource.key === 'pricing')!;
    render(<ResourcePanel hostId="host-a" resource={pricing}/>);

    await userEvent.click(await screen.findByRole('button',{name:'Create pricing version'}));
    expect(mocks.navigate).toHaveBeenCalledWith('/app/form/createPricingVersion',{
