...
export default function PublicationPanel({hostId}: {hostId: string}) {
  ...
  const generateCandidate = async () => {
    ...
    try {
      ...
      setMessage({severity:'info',text:'Generated from current active records. Portal has not contacted the provider; test after creating a snapshot and restarting or reloading the selected gateway.'});
    } catch (reason) {
  ...
  const applyExactRevision = async (item: RecordValue) => {
    ...
    try {
      ...
      setMessage({severity:'success',text:'The exact stored revision was reapplied. Create and promote a new config snapshot before restarting or reloading llm-router.'});
      await loadHistory();
    } catch (reason) { setMessage({severity:'error',text:llmErrorMessage(reason)}); }
    finally { setLoading(false); }
  };
