...
export default function ToolParam() {
    ...
    const handleDelete = useCallback(async (row: MRT_Row<ToolParamType>) => {
        ...
        setData(prev => prev.filter(d => d.paramId !== row.original.paramId));
        setRowCount(prev => prev - 1);

        const cmd = {
            host: 'lightapi.net', service: 'genai', action: 'deleteToolParam', version: '0.1.0',
      data: { hostId: row.original.hostId, paramId: row.original.paramId , aggregateVersion: row.original.aggregateVersion},
        };
        ...
        try {
            ...
            alert('Failed to delete parameter due to a network error.');
            setData(originalData);
            setRowCount(originalData.length);
        }
    }, [data]);
    ...
    const handleUpdate = useCallback(async (row: MRT_Row<ToolParamType>) => {
      ...
      data: { hostId: row.original.hostId, aggregateVersion: row.original.aggregateVersion, paramId: row.original.paramId },
        };
        const url = '/portal/query?cmd=' + encodeURIComponent(JSON.stringify(cmd));
      ...
      const dataForForm = freshData.aggregateVersion === row.original.aggregateVersion ? row.original : freshData;
            ...
            console.error("Failed to fetch parameter for update:", error);
            alert("Could not load the latest parameter data. Please try again.");
        } finally {
    ...
    const columns = useMemo<MRT_ColumnDef<ToolParamType>[]>(
        () => [
            ...
            { accessorKey: 'validationSchema', header: 'Validation Schema' },
            { accessorKey: 'orderIndex', header: 'Order Index' },
            ...
            <Box sx={{ display: 'flex', gap: '1rem' }}>
                <Tooltip title="Update Parameter">
                    <IconButton
                        ...
                        disabled={isUpdateLoading === row.original.paramId}
                    >
                        {isUpdateLoading === row.original.paramId ? (
