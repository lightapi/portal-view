...
// --- Type Definitions ---
type SkillDependencyApiResponse = {
    skillDependencies: Array<SkillDependencyType>;
    total: number;
};
...
export default function SkillDependency() {
    ...
    const contextForRow = useCallback(
        (row: SkillDependencyType) => buildGenAiTaskContext(host, searchParams, row),
        [host, searchParams],
    );
    ...
    const fetchData = useCallback(async () => {
        ...
        const cmd = {
            host: 'lightapi.net', service: 'genai', action: 'getSkillDependency', version: '0.1.0',
            data: {
    ...
    const handleDelete = useCallback(async (row: MRT_Row<SkillDependencyType>) => {
        if (!window.confirm(`Are you sure you want to delete dependency of ${row.original.skillId} on ${row.original.dependsOnSkillId}?`)) return;

        const originalData = [...data];
        setData(prev => prev.filter(d => d.skillId !== row.original.skillId || d.dependsOnSkillId !== row.original.dependsOnSkillId));
        ...
        try {
            ...
            alert('Failed to delete dependency due to a network error.');
            setData(originalData);
            setRowCount(originalData.length);
        }
    }, [data]);

    const handleUpdate = useCallback(async (row: MRT_Row<SkillDependencyType>) => {
        ...
        try {
            ...
            console.error("Failed to fetch dependency for update:", error);
            alert("Could not load the latest dependency data. Please try again.");
        } finally {
    ...
    // Column definitions
    const columns = useMemo<MRT_ColumnDef<SkillDependencyType>[]>(
        () => [
            { accessorKey: 'hostId', header: 'Host Id' },
            { accessorKey: 'skillId', header: 'Skill Id' },
            { accessorKey: 'dependsOnSkillId', header: 'Depends On Skill Id' },
            {
    ...
    const table = useMaterialReactTable({
        ...
        renderTopToolbarCustomActions: () => (
            <Button variant="contained" startIcon={<AddBoxIcon />} onClick={() => navigate(buildGenAiTaskRoute('/app/form/createSkillDependency', searchParams, taskContext))}>
