...
export default function AgentSkill() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const json = (await response.json()) as AgentSkillApiResponse;
    ...
    const handleUpdate = useCallback(async (row: MRT_Row<AgentSkillType>) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(freshData.description || 'Failed to fetch latest agent skill data.');
            }
    ...
    const columns = useMemo<MRT_ColumnDef<AgentSkillType>[]>(
        () => [
            ...
            },
            {
                ...
                Cell: ({ row }) => (<Tooltip title="Delete Agent Skill"><IconButton color="error" onClick={() => handleDelete(row)}><DeleteForeverIcon /></IconButton></Tooltip>),
            },
        ],
