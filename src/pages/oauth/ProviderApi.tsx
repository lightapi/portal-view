...
type ProviderApiResponse = {
  authProviderApis: Array<ProviderApiType>; // Assuming API response key
  total: number;
};
...
export default function ProviderApi() {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<ProviderApiType>) => {
    if (!window.confirm(`Are you sure you want to remove API ${row.original.apiId} from this provider?`)) return;

    const originalData = [...data];
    setData(prev => prev.filter(pa => !(pa.providerId === row.original.providerId && pa.apiId === row.original.apiId)));
    ...
    try {
      ...
      if (result.error) {
        alert('Failed to remove API from provider. Please try again.');
        setData(originalData);
        setRowCount(originalData.length);
      }
    } catch (e) {
      alert('Failed to remove API from provider due to a network error.');
      setData(originalData);
      setRowCount(originalData.length);
    }
  }, [data]);
  ...
  // Column definitions
  const columns = useMemo<MRT_ColumnDef<ProviderApiType>[]>(
    () => [
  ...
  const table = useMaterialReactTable(usePortalActionTableOptions({
    ...
    renderRowActions: ({ row }) => <PortalActions row={row} actions={[
      {
        id: "remove-api-from-provider",
        label: "Remove API from Provider",
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        {initialProviderId && (
          <Typography variant="subtitle1">
            APIs for Provider: <strong>{initialProviderId}</strong>
          </Typography>
        )}
      </Box>
    ),
