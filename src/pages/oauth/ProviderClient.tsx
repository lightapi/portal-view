...
export default function ProviderClient() {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<ProviderClientType>) => {
    ...
    const cmd = {
      ...
      data: { hostId: row.original.hostId, providerId: row.original.providerId, clientId: row.original.clientId , aggregateVersion: row.original.aggregateVersion},
    };
  ...
  const table = useMaterialReactTable(usePortalActionTableOptions({
    ...
    renderRowActions: ({ row }) => <PortalActions row={row} actions={[
      {
        id: "remove-client-from-provider",
        label: "Remove Client from Provider",
