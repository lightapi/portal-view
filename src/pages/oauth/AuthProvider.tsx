...
function Row(props) {
  ...
  const handleKeys = (provider) => {
    navigate("/app/oauth/providerKey", { state: { data: { ...provider } } });
  };

  const handleServices = (provider) => {
    navigate("/app/oauth/updateProvider", { state: { data: { ...provider } } });
  };

  const handleClients = (provider) => {
    navigate("/app/form/updateProvider", { state: { data: { ...provider } } });
  };
  ...
  return (
    <TableRow className={classes.root}>
      ...
      <TableCell align="right">
        <KeyIcon onClick={() => handleKeys(row)} />
      </TableCell>
      <TableCell align="right">
        <ApiIcon onClick={() => handleServices(row)} />
      </TableCell>
      <TableCell align="right">
        <AppsIcon onClick={() => handleClients(row)} />
      </TableCell>
    </TableRow>
  );
}
