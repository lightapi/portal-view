...
export function ReplayWaiverPanel({ failureIds, currentUserId, busy, error, orderedScopeSelected, onRequest, onApprove }:
  { failureIds: string[]; currentUserId?: string | null; busy: boolean; error?: string | null; orderedScopeSelected?: boolean;
  ...
  return <Paper variant="outlined" sx={{ p: 2 }}><Stack spacing={1}>
    ...
    {orderedScopeSelected ? <Alert severity="error">
      Ordered aggregate and graph-root failures cannot be waived because that would leave a permanent projection-version gap. Repair or replay the failure instead.
    </Alert> : null}
    ...
    <Stack direction="row" spacing={1}>
      <Button variant="outlined" disabled={busy || orderedScopeSelected || !confirmed || !reason.trim() || !!request} onClick={async () => {
      ...
      <Button color="warning" variant="contained" disabled={busy || orderedScopeSelected || !confirmed || !reason.trim() || !approvalId || requestedBy === currentUserId}
