...
export default function ProcessInfo() {
    ...
    const contextForRow = useCallback(
        (row: ProcessInfoType) => buildWorkflowTaskContext(host, searchParams, row),
        [host, searchParams],
    );
    ...
    const handleDelete = useCallback(async (row: MRT_Row<ProcessInfoType>) => {
      ...
      data: { hostId: row.original.hostId, processId: row.original.processId , aggregateVersion: row.original.aggregateVersion},
        };
    ...
    const table = useMaterialReactTable(usePortalActionTableOptions({
      ...
      renderRowActions: ({ row }) => <PortalActions row={row} actions={[
        {
          id: "update-process-info",
          label: "Update Process Info",
          icon: (
            <SystemUpdateIcon />
          ),

          loading: () => Boolean(isUpdateLoading === row.original.processId),
          onSelect: () => handleUpdate(row)
        },
        {
          id: "delete-process-info",
          label: "Delete Process Info",
        ...
        renderTopToolbarCustomActions: () => (
            <Button variant="contained" startIcon={<AddBoxIcon />} onClick={() => navigate(buildWorkflowTaskRoute('/app/form/createProcessInfo', searchParams, taskContext))}>
