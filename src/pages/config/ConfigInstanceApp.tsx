...
export default function ConfigInstanceApp() {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<ConfigInstanceAppType>) => {
    ...
    const cmd = {
      ...
      data: { hostId: row.original.hostId, instanceAppId: row.original.instanceAppId, propertyId: row.original.propertyId , aggregateVersion: row.original.aggregateVersion},
    };
  ...
  const table = useMaterialReactTable(usePortalActionTableOptions({
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
        <Tooltip describeChild title={!initialInstanceAppId ? 'Select an instance application to update its configuration.' : ''}>
