  ...
  const columns = useMemo<MRT_ColumnDef<ConfigInstanceAppApiType>[]>(
    () => [
      ...
      { accessorKey: 'propertyValue', header: 'Property Value' },
      {
        ...
        Cell: ({ row }) => (<Tooltip title="Update Property"><IconButton onClick={() => navigate('/app/form/updateConfigInstanceAppApi', { state: { data: { ...row.original } } })}><SystemUpdateIcon /></IconButton></Tooltip>),
      },
      {
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <Button
          ...
          onClick={() => navigate('/app/form/createConfigInstanceAppApi', { state: { data: initialData } })}
        >
          Add Property
        </Button>
