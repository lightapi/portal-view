...
export default function ConfigDeploymentInstance() {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<ConfigDeploymentInstanceType>) => {
    if (!window.confirm(`Are you sure you want to delete this property from the deployment instance?`)) return;
  ...
  const handleUpdate = useCallback(async (row: MRT_Row<ConfigDeploymentInstanceType>) => {
    const propertyId = row.original.propertyId;
    setIsUpdateLoading(propertyId);

    const cmd = {
      host: 'lightapi.net', service: 'config', action: 'getFreshConfigDeploymentInstance', version: '0.1.0',
      data: row.original,
    };
    const url = '/portal/query?cmd=' + encodeURIComponent(JSON.stringify(cmd));
    const cookies = new Cookies();
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(freshData.description || 'Failed to fetch latest config deployment instance property data.');
      }
      ...
      console.error("Failed to fetch config deployment instance property for update:", error);
      alert("Could not load the latest config deployment instance property data. Please try again.");
    } finally {
