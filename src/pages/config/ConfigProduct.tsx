...
export default function ConfigProduct() {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<ConfigProductType>) => {
    ...
    const cmd = {
      ...
      data: { productId: row.original.productId, propertyId: row.original.propertyId , aggregateVersion: row.original.aggregateVersion},
    };
  ...
  const handleUpdate = useCallback(async (row: MRT_Row<ConfigProductType>) => {
    ...
    try {
      ...
      navigate(buildTaskAwareRoute('/app/form/updateConfigProduct', searchParams, {
        ...taskContext,
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <Button
          ...
          onClick={() => navigate(
            buildTaskAwareRoute('/app/form/createConfigProduct', searchParams, taskContext),
