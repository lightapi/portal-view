  ...
  const columns = useMemo<MRT_ColumnDef<ConfigProductVersionType>[]>(
    () => [
      ...
      { accessorKey: 'productId', header: 'Product ID' },
      {
        ...
        Cell: ({ row }) => (<Tooltip title="Update Property"><IconButton onClick={() => navigate('/app/form/updateConfigProductVersion', { state: { data: { ...row.original } } })}><SystemUpdateIcon /></IconButton></Tooltip>),
      },
      {
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <Button
          ...
          onClick={() => navigate('/app/form/createConfigProductVersion', { state: { data: initialData } })}
          disabled={!initialData.configId}
        >
          Add Property
        </Button>
        {initialData.configId && (
          <Typography variant="subtitle1">
            For Config: <strong>{initialData.configId}</strong>
          </Typography>
        )}
      </Box>
    ),
