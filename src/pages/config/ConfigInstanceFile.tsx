...
export default function ConfigInstanceFile() {
  ...
  const taskContext = useMemo(
    () => mergeTaskContext(searchContext, {
      ...
      instanceFileId: searchContext.instanceFileId ?? '',
    }),
  ...
  const handleDelete = useCallback(async (row: MRT_Row<ConfigInstanceFileType>) => {
    ...
    const cmd = {
      ...
      data: {
        ...
        instanceFileId: row.original.instanceFileId,
  ...
  const handleUpdate = useCallback(async (row: MRT_Row<ConfigInstanceFileType>) => {
    ...
    const cmd = {
      ...
      data: { hostId: row.original.hostId, instanceFileId: row.original.instanceFileId, aggregateVersion: row.original.aggregateVersion },
    };
    const url = '/portal/query?cmd=' + encodeURIComponent(JSON.stringify(cmd));
    try {
      ...
      navigate(buildTaskAwareRoute('/app/form/updateConfigInstanceFile', searchParams, {
        ...taskContext,
        instanceId,
        instanceFileId: row.original.instanceFileId,
      }), {
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <Button
          ...
          onClick={() => navigate(
            buildTaskAwareRoute('/app/form/createConfigInstanceFile', searchParams, taskContext),
