...
import { platformLoadErrorMessage } from './deploymentLoadErrorMessage';
...
export default function PlatformAdmin() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const json = await fetchClient(url) as PlatformApiResponse;
      setData(json.platforms || []);
      setRowCount(json.total || 0);
    } catch (error) {
      setLoadError(platformLoadErrorMessage(error));
      console.error(error);
    } finally {
  ...
  const handleDelete = useCallback(async (row: MRT_Row<PlatformType>) => {
    ...
    const cmd = {
      ...
      data: { hostId: row.original.hostId, platformId: row.original.platformId , aggregateVersion: row.original.aggregateVersion},
    };
